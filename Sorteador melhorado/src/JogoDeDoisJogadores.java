...
public abstract class JogoDeDoisJogadores {
    ...
    private float getPercentual(int option) {
        float p = 0;
        float numRodadas = 0;

        for(Partida partida : historicoResultados) {
            if(option == 1)
                p += partida.contRodadasVencidasJogador1;
            else if(option == 2)
                p += partida.contRodadasVencidasJogador2;
            else
                p += partida.contEmpates;
            
            numRodadas += partida.contEmpates + partida.contRodadasVencidasJogador1 + partida.contRodadasVencidasJogador2;
        }

        return (p/numRodadas)*100;
    }
